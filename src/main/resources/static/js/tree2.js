...
Tree.prototype.onCheckClick = function (target) {
	var onChange = this.options.onChange;

	
	onChange && onChange.call(this);
	console.timeEnd('onCheckClick');
};

Tree.prototype.onItemClick = function (target) {
	console.time('onItemClick');
	var nodeId = target.nodeId;
	var node = this.nodesById[target.nodeId];
	var onChange = this.options.onChange;
	console.log("target:",target);
	
	if (!node.disabled) {
		this.setValue(target.nodeId);
